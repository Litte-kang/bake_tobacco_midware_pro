...
function proRemoteCmd(serPort, serIP, cmdInfos)
{
	...
	if (0 === isProCmd)
	{
		...
		event.on('pro_cmd', function(cmdInfo){
			...
			else if (12 === cmdInfo.type) //-- none standard cmd --//
			{
	...
	{	
		...
		sendResponseInfo(serPort, serIP, errCode);
	}	
}

/***********************************************************************
**Function Name	: parseCurveCmd
**Description	: convert a none standard cmd to standard cmd.
**Parameters	: cmdInfo {Json} - in.
...
function parseCurveCmd(cmdInfo)
{
	...
	standarCmd.type = cmdInfo.type;
	standarCmd.address = cmdInfo.address;
	standarCmd.data.push(cmdInfo.data[0]);
	standarCmd.data.push(cmdInfo.data[1]);
	
	for(i = 0; i < cmdInfo.data[2].dryBulbCurve.length; ++i)
	{
		standarCmd.data.push(cmdInfo.data[2].dryBulbCurve[i]);
	}

	for(i = 0; i < cmdInfo.data[3].wetBulbCurve.length; ++i)
	{
		tmp = cmdInfo.data[3].wetBulbCurve[i] * 10;
		standarCmd.data.push(((tmp >> 8) & 0x00ff));
		standarCmd.data.push((tmp & 0x00ff));
	}

	for(i = 0; i < cmdInfo.data[4].timeCurve.length; ++i)
	{
		standarCmd.data.push(cmdInfo.data[4].timeCurve[i]);
	}
			...
			{	
				...
				if (0 !== curVer.length)
				{
					...
					{	
						...
						ack.data[0] = 1;
						sendResponseInfo(serPort, serIP, ack);
					...
					else if (fwInfos.fwVer[1] < fwInfos.fwVer[0])
					{
						...
						sendResponseInfo(serPort, serIP, ack);	//-- notice server --//	
...
function startDownloadFw(fwInfos)
{
	var ack = 
	{
		type:4,
		address:MIDWARE_ID,			
		data:[0/*0 download failed, 1 - download ok, 2 - ver is not latest*/, fwInfos.fwType]
	};
	...
	fw.Event.once('download_end', function(){
		...
		sendResponseInfo(fwInfos.serPort, fwInfos.serIP, ack);		
...
function startUpInnerCommu(serPort, serIP, cmdInfo, innerPort)
{
	...
	innerCommu.Event.once('communication_end', function(){
		...
		if (15 === cmdInfo.type)
		{			
			var kill_cprocess = spawn('/bin/bash', ['./node_modules/remote_cmd/kill_cprocess', 'Bake_Tobacco_Monitor'], {stdio:'inherit'});
...
**Return		: none.
***********************************************************************/
function sendResponseInfo(serPort, serIP, resInfo)
{
